...
if [ "${SLMK}" == "true" ]; then
zip -r9 personal-oldcam-exteas-slmk${UCLAMP_NAME}-${DATE}.zip *
else
zip -r9 personal-oldcam-exteas${UCLAMP_NAME}-${DATE}.zip *
fi
elif [ "${CONFIG}" == "whyred-newcam_defconfig" ]; then
zip -r9 personal-newcam-radeas${UCLAMP_NAME}-${DATE}.zip *
elif [ "${CONFIG}" == "extfake_defconfig" ]; then
zip -r9 personal-extfake${UCLAMP_NAME}-${DATE}.zip *
fi
cd ..
END=$(date +"%s")
DIFF=$(($END - $START))
